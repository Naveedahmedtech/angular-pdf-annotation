...
export class RXCore {
  ...
  public static setBirdseyeCanvas(
    imagecanvas,
    indicatorcanvas,
    markupcanvas
  ): void {
  ...
  public static markupButtonFromMatch(
    rectanle,
    pagenumber,
    image,
    padding
 ...
 */
  ...
  public static uploadFile(file: File): Promise<any> {
    return new Promise(function (resolve, reject) {
      ...
      xhr.open(
        'POST',
        RxConfig.FileuploadURL +
          '&' +
          RxConfig.UploadServerfolder +
          '&' +
          file.name +
          '&' +
          file.lastModified +
  ...
  public static async compareOverlayServerJSON(
    ...
    if (
      backFileName?.toLowerCase().endsWith('dwg') ||
    ...
    const payload = [
      {
        ...
        BackURL: `${RXCore.Config.UploadServerfolder.replaceAll(
          '\\',
          '/'
        )}${backFileName}`,
        FrontURL: `${RXCore.Config.UploadServerfolder.replaceAll(
          '\\',
          '/'
        )}${frontFileName}`,
        CompareDPI: dpi,
  ...
  public static uploadSignature(
    PNGImage: any,
    username: string,
    binitial: boolean = false
  ): void {
  ...
  public static onGuiMarkupList(
    callback: (list: Array<IMarkup>) => void
  ): void {
  ...
  public static onGuiMarkupHover(
    callback: (markup: IMarkup, x: number, y: number) => void
  ): void {
  ...
  public static onGuiVectorLayers(
    callback: (layers: Array<IVectorLayer>) => void
  ): void {
    RxCore.GUI_VectorLayers.connect(callback);
  }

  public static onGuiVectorBlocks(
    callback: (layers: Array<IVectorBlock>) => void
  ): void {
  ...
  public static onGuiPagethumbs(
    callback: (thumbnails: Array<IPageThumb>) => void
  ): void {
  ...
  public static onGuiPutSignatureComplete(
    callback: (username: string) => void
  ): void {
    RxCore.GUI_putsignatureComplete.connect(callback);
  }

  public static onGuiGetSignatureComplete(
    callback: (signature: ISignatureData) => void
  ): void {
    RxCore.GUI_getsignatureComplete.connect(callback);
  }

  public static onGuiPanUpdated(
    callback: (sx: number, sy: number, pagerect: any) => void
  ): void {
    RxCore.GUI_PanUpdated.connect(callback);
  }

  public static onGuiZoomUpdate(
    callback: (zoomparams: any, type: number) => void
  ): void {
